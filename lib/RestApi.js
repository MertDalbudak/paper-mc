    ...
    */
    constructor(serviceName, endpointName, options = {'params': {}, 'queries': {}, 'headers': {}}){
        ...
        if(this.validateInputs(serviceName, endpointName, options) != true){
            console.log(`RestApi Validation of ${serviceName}: ${endpointName} failed`, 'RestApi');
            return;
        }
        // LOAD REST SERVICE
        ...
        this.authorized.then((auth_header)=> {
            ...
            console.error(error);
            console.log(`RestApi authorization of ${serviceName}: ${endpointName} failed. Authorization type was ${service.authorization.type}`, 'RestApi');
        });
    }
    ...
    validateInputs(serviceName, endpointName, options){
        ...
        console.log(`RestApi Validation of ${serviceName}: ${endpointName} completed`, 'RestApi');
        return true;
    }
                ...
                await (new Promise((resolve, reject)=>{
                    oauth2.getOAuthAccessToken('', auth.params, (error, access_token, refresh_token, results) =>{
                        if(error == null){
                            ...
                            console.log(error, `OAuth2(${this.serviceName}:${this.endpointName})`);
                            reject(error);
                        }
                    });
                }));
            ...
            this.api_request = request(this.options, async res => {
                ...
                res.on('data', (data)=> {
                    if(typeof data == 'string' || Buffer.isBuffer(data))
                        ...
                        console.log(data, "Chunk is not String or Buffer", 'request');
                    }
                });
                res.on('end', ()=> {
